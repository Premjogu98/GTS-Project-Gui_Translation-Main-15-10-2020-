...
def translation():
    try:
        trasns = connection()
        cur = trasns.cursor()
        # cur.execute(f"SELECT * FROM `tenders_db`.`l2l_tenders_tbl` WHERE source='tenders.etimad.sa' AND is_english = '1'")  # For test
        ...
        for row in rows:
            try:
                ...
                print('Error On translation Function\n')
                Print_Exception_detail(e)
                try:
                    browser.get('https://translate.google.com/')
                except:
        ...
        print('Error On Main Exception Function\n')
        Print_Exception_detail(e)
